...
<a id="log-2026-09-01"></a>

## 2026.9.1

### 完成内容

完整实现了 `06_attention.cu`。
...
![Attention correctness 与 benchmark 结果](images/image-29.png)

所有 Naive、Tiled、Causal Naive 和 Causal Tiled correctness 测试均通过。

| Kernel | 平均时间 | 相对 Naive 性能 |
| --- | ---: | ---: |
| Naive | 0.734201 ms | 1.000× |
| Tiled | 0.921953 ms | 0.796× |

Tiled 比 Naive 慢约 25.6%。

### Tiled Profile

![Tiled Attention 的 Nsight Compute 基础结果](images/image-27.png)
![Tiled Attention 的 Nsight Compute 计算与调度结果](images/image-28.png)

#### Profile 证据

| 观察维度 | Profile 指标 | 数值 | 证据说明 |
| --- | --- | ---: | --- |
| 显存利用 | DRAM Throughput | 0.30% | 当前 kernel 不是 DRAM bandwidth-bound；消除 scores 和 probabilities 的 global-memory 往返后，显存流量已很低。 |
| Shared memory 占用 | Static Shared Memory Per Block | 16.92 KB | 每个 block 使用较多 shared memory，使每个 SM 最多驻留 5 个 block。 |
| Occupancy | Theoretical / Achieved Occupancy | 20.83% / 19.31% | 低 occupancy 减少了可用于隐藏延迟的 active warps。 |
| Warp 有效线程 | Avg. Not Predicated Off Threads Per Warp | 4.55 / 32 | 每个 warp 平均只有约 14.2% 的 lane 执行有效指令，反映单线程 score 计算造成大量线程闲置。 |
| 调度状态 | No Eligible | 69.74% | Scheduler 在大部分周期没有可发射下一条指令的 warp。 |
| 指令发射 | Issue Slots Busy | 28.97% | 大部分 issue slot 未被利用，计算流水线没有被充分喂满。 |
| 同步等待 | CTA Barrier Stall | 45.71% | 其他线程频繁等待串行计算 score 的线程到达 block barrier。 |

Tiled-fused Attention 成功消除了 scores 和 probabilities 的 global-memory 往返，DRAM throughput 仅为 0.30%，但它没有实现 FlashAttention 所需的并行 score-tile 计算。当前实现由单个线程串行计算每个 score tile，使每个 warp 平均只有 4.55 个线程执行有效工作，并导致其他线程在 block barrier 处等待。与此同时，约 16.92 KB 的 shared memory 将理论 occupancy 限制为 20.83%。最终 scheduler 在 69.74% 的周期中没有 eligible warp，Issue Slots Busy 仅为 28.97%。因此，单线程计算、barrier 等待和低 occupancy 共同抵消了减少 global-memory traffic 的收益。

### 结论

当前 Tiled kernel 在算法层面使用了 FlashAttention 的 IO-aware 和 online-softmax 思想，但没有实现其关键的 tile-level 并行计算与跨 query-row 数据复用。节省 global-memory 流量的收益，被单线程 score 计算、同步和线程闲置成本抵消，所以反而比 Naive 慢约 25.6%。
